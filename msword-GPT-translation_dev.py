...
def translate_docx(input_path, output_path, source_lang, target_lang, custom_prompt, model, temperature):
    ...
    max_token_limit = 4096 // 3
    paragraphs_to_translate = []
    total_tokens = 0

    def add_translated_paragraphs(paragraphs, styles):
        for text, style in zip(paragraphs, styles):
            ...
            new_para = translated_doc.add_paragraph(translated_text, style=style)
    ...
    for paragraph in doc.paragraphs:
        ...
        if total_tokens + tokens <= max_token_limit and text:
            paragraphs_to_translate.append(text)
            para_styles.append(paragraph.style)
            total_tokens += tokens
        else:
            # Translate and add the paragraphs accumulated so far
            add_translated_paragraphs(paragraphs_to_translate, para_styles)

            # Start a new batch of paragraphs
            paragraphs_to_translate = [text] if text else []
            para_styles = [paragraph.style] if text else []
            total_tokens = tokens if text else 0

    # Translate and add the remaining paragraphs
    add_translated_paragraphs(paragraphs_to_translate, para_styles)
...
def get_input_file():
    parser = argparse.ArgumentParser(description="Translate a Word document using Openai GPT API.")
